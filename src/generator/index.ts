...
export class GeneratorModule {
  ...
  private async scaffoldProject(outputDir: string, analysis: SaaSAnalysis, slug: string): Promise<void> {
    ...
    // Use visual design colors if available, otherwise default Tailwind config
    if (analysis.visualDesign) {
      const customTailwind = this.generateVisualTailwindConfig(analysis.visualDesign);
      await fs.writeFile(path.join(outputDir, 'apps/web/tailwind.config.js'), customTailwind);
    } else {
...
`;
  ...
  /**
   * Generate Tailwind config based on visual design analysis
   */
  private generateVisualTailwindConfig(visualDesign: any): string {
    const primary = visualDesign.primaryColor || '#3b82f6';
    const secondary = visualDesign.secondaryColor || '#64748b';
    const accent = visualDesign.accentColor || '#8b5cf6';
    const bg = visualDesign.backgroundColor || '#ffffff';
    const text = visualDesign.textColor || '#1f2937';
    const font = visualDesign.fontFamily || 'Inter';
...
module.exports = {
  ...
  theme: {
    extend: {
      ...
      borderRadius: {
        DEFAULT: '${visualDesign.designTokens?.borderRadius || '0.5rem'}',
      },
    },
  },
  plugins: [],

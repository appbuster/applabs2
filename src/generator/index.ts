...
export class GeneratorModule {
  ...
  private async scaffoldProject(outputDir: string, analysis: SaaSAnalysis, slug: string): Promise<void> {
    ...
    // Generate ORIGINAL design - if we have target's visual design, generate something DIFFERENT
    // This avoids exact copying and potential lawsuits
    if (analysis.visualDesign) {
      const originalTailwind = this.generateOriginalTailwindConfig(analysis.visualDesign);
      await fs.writeFile(path.join(outputDir, 'apps/web/tailwind.config.js'), originalTailwind);
      logger.info('Generated original design that differs from target');
    } else {
...
`;
  ...
  /**
   * Generate ORIGINAL Tailwind config that DIFFERS from target
   * Takes the target's design and creates a contrasting/different design
   */
  private generateOriginalTailwindConfig(targetDesign: any): string {
    // Generate DIFFERENT colors from target to avoid copying
    const alternativeColors = [
      { primary: '#6366f1', name: 'Indigo' },    // Indigo
      { primary: '#8b5cf6', name: 'Violet' },    // Violet
      { primary: '#ec4899', name: 'Pink' },      // Pink
      { primary: '#14b8a6', name: 'Teal' },      // Teal
      { primary: '#f59e0b', name: 'Amber' },     // Amber
      { primary: '#10b981', name: 'Emerald' },   // Emerald
      { primary: '#3b82f6', name: 'Blue' },      // Blue
      { primary: '#ef4444', name: 'Red' },       // Red
    ];
    
    // Pick a color that's different from target
    const targetPrimary = targetDesign.primaryColor?.toLowerCase() || '#3b82f6';
    let selectedColor = alternativeColors[0];
    for (const alt of alternativeColors) {
      if (!this.colorsAreSimilar(alt.primary, targetPrimary)) {
        selectedColor = alt;
        break;
      }
    }
    
    // Use different layout hint
    const targetLayout = targetDesign.layoutType || 'sidebar-left';
    
    // Pick different font from target
    const targetFont = targetDesign.fontFamily?.toLowerCase() || 'inter';
    const fonts = ['Inter', 'Poppins', 'DM Sans', 'Plus Jakarta Sans', 'Outfit'];
    const selectedFont = fonts.find(f => f.toLowerCase() !== targetFont) || 'Inter';
    
    logger.info(`Target: ${targetPrimary}, We use: ${selectedColor.primary} (${selectedColor.name})`);
    logger.info(`Target font: ${targetFont}, We use: ${selectedFont}`);
    
    return `/** @type {import('tailwindcss').Config} */
// ORIGINAL DESIGN - intentionally different from target
module.exports = {
  ...
  theme: {
    extend: {
      colors: {
        primary: {
          DEFAULT: '${selectedColor.primary}',
          50: '${this.lightenColor(selectedColor.primary, 90)}',
          100: '${this.lightenColor(selectedColor.primary, 80)}',
          200: '${this.lightenColor(selectedColor.primary, 60)}',
          500: '${selectedColor.primary}',
          600: '${this.darkenColor(selectedColor.primary, 10)}',
          700: '${this.darkenColor(selectedColor.primary, 20)}',
        },
        secondary: {
          DEFAULT: '#64748b',
          500: '#64748b',
      ...
      fontFamily: {
        sans: ['${selectedFont}', 'system-ui', '-apple-system', 'sans-serif'],
      },
      borderRadius: {
        DEFAULT: '0.75rem',
        lg: '1rem',
...
`;
  ...
  /**
   * Check if two colors are similar (to avoid using similar colors)
   */
  private colorsAreSimilar(color1: string, color2: string): boolean {
    // Simple check - compare hue roughly
    ...
    return distance < 100; // Colors are similar if distance < 100
  }
  ...
  /**
   * Simple color darkening (for button hover states etc)
   */
